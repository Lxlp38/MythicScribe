...
export function getAttributeDataByName(
    mechanic: Mechanic,
    attributeName: string,
    type: ObjectType,
) {
    const attribute = mechanic.attributes.find((attr: Attribute) =>
        includesIgnoreCase(attr.name, attributeName),
    );
...
function getInheritedAttributeDataByName(
    mechanic: Mechanic,
    attributeName: string,
    type: ObjectType,
) {
    const attribute = mechanic.attributes.find(
        (attr: Attribute) =>
            attributeIsInheritable(attr) && includesIgnoreCase(attr.name, attributeName),
    );
