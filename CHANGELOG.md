...
### Changed
- When the `editor.acceptSuggestionOnEnter` config is set to anything but `false` or `smart`, file-specific suggestions (the suggestions triggered when pressing enter) will not pop up automatically. They can, of course, still be programmatically called by using `ctrl+space`.  
